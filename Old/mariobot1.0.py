...
class MarioBot(irc.bot.SingleServerIRCBot):
    ...
    def __init__(self, channel, nickname='mario', server='irc.rd.tandberg.com', port=6667):
        ...
        self.more_players_re = re.compile('mario: ([123]) more')#[|in (\d hours)|in (\d+ minutes)]')
        self.yes_for_re = re.compile('mario: (?:{}) for ([a-zA-Z]+)'.format('|'.join(self.possible_yes)))
    ...
    def check_and_answer(self, e, text):
        ...
        lfgre = self.more_players_re.match(text) ### 'mario: X more'
        if lfgre:
            if 'trains' in source:
                ...
                #if len(lfgre.groups()) > 1:
                #    timetil = lfgre.groups()[1]
                #    self.timer = int(lfg)
        ...
        #if (text == 'mario: yes' or text == 'mario: aye' or text == 'mario: yeah' or text == 'mario: y' or text == 'mario: yup') and 1 <= self.required <= 3:
            #if 'rwge' in source:
            #    c.privmsg(self.channel,'Go away, rwge.')
            if 'clvr' == source:
        ...
        if text == 'mario: check':
            if self.isBooked:
                c.privmsg(self.channel, 'Booked recently by %s' % self.booker)
                self.ready_players = []
            else:
        ...
        if self.allow_eggs:
            ...
            #        
            #if text == 'mario: nay':
            #    c.privmsg(self.channel,'AYE.')
                    
            if text == 'mario: crash':
                c.privmsg(self.channel,'gack *dies*')
                #c.quit(message = 'Read error: Connection reset by peer')
                #time.sleep(1)
                #c.join(self.channel)
                #time.sleep(10)
            ...
            if text == 'mario: dance' and time.time() - self.dancetimer > 60:
                ...
                for msg in msglist:
                    c.privmsg(self.channel,msg)
                    time.sleep(0.5)
                #c.privmsg(self.channel,'NNtsssNNtsssNNtsssNNtsssNNtsssNNtsssNNtsssNNtsss YEAAH!')
