...
class LalBot(object):
    ...
    def on_pubmsg(self,source,text,private=False):
        '''Respond to a message sent - choose from self._command_list'''
        
        print u'RECV {}:{}: {}'.format('Priv' if private else 'Pub',source,text)
        
        nick = self.nickname
        command_re = re.compile('(?i){}: (.+)'.format(nick))
    ...
    def on_privmsg(self,connection,event):
        if not event.startswith(self.nickname + ': '):
            event = self.nickname + ': ' + event ### Test this!
